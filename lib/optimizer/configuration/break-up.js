...
function transition(property, configuration, validator) {
  ...
  var timing = _wrapDefault(property.name + '-timing-function', property, configuration);
  var delay = _wrapDefault(property.name + '-delay', property, configuration);
  var behavior = _wrapDefault(property.name + '-behavior', property, configuration);
  var components = [prop, duration, timing, delay, behavior];
  var values = property.value;
  var value;
  var durationSet = false;
  var delaySet = false;
  var propSet = false;
  var timingSet = false;
  var behaviorSet = false;
  var i;
  var l;

  if (property.value.length == 1 && property.value[0][1] == 'inherit') {
    prop.value = duration.value = timing.value = delay.value = behavior.value = property.value;
    return components;
  }
  ...
  for (i = 0, l = values.length; i < l; i++) {
    ...
    if (validator.isTime(value[1]) && !durationSet) {
      ...
      prop.value = [value];
      propSet = true;
    } else if (validator.isIdentifier(value[1]) && !behaviorSet) {
