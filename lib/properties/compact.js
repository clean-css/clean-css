    ...
    var mergeSame = function(tokens) {
        ...
        for (var i = 0; i < tokens.length; i++) {
            ...
            if (processable[token.prop] && processable[token.prop].alwaysMergeRedefined) {
                ...
                var alreadyMerged = result.filter(function(t) { return t.prop === token.prop && t.value === token.value; });
                if (alreadyMerged.length === 0) {
                    result.push(token);
                }
                else if (token.isImportant) {
                    // foo{bar:baz !important;bar:baz} -> foo{bar:baz !important}
                    alreadyMerged[0].isImportant = true;
                }
