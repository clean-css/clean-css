    ...
    bySpacesOmitDefaults: function (prop, tokens, isImportant) {
      ...
      for (var i = 0; i < tokens.length; i++) {
        ...
        if (processable[token.prop] && processable[token.prop].defaultValue === token.value) {
          result.defaultsOut = result.defaultsOut || { };
  ...
  var compactToShorthands = function(tokens, important) {
    ...
    for (var i in processable) {
      ...
      while (true) {
        ...
        var trickComps = [];
        // Number of real components found
        var realComponentsCount = 0;
        ...
        for (var j = 0; j < sh.components.length; j++) {
          ...
          else if (lastCompacted && lastCompacted.defaultsOut && lastCompacted.defaultsOut[cc]) {
            // Another trick: when a previous shorthand defaults out a certain granular property, we can
            // hack together another shorthand if doesn't contain said granular property.
            comps.push(makeDefaultProperty(cc));
          }
          else {
        ...
        if (realComponentsCount === 0 || comps.length < sh.components.length) {
        ...
        if (trickComps.length === 0 || detokenize([compacted]).length <= detokenize(comps).length || comps.some(isHackValue)) {
          ...
          for (var k = 1; k < comps.length; k++) {
            var index = tokens.indexOf(comps[k]);
            if (index >= 0) {
