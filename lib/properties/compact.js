...
module.exports = (function () {
  ...
  //   Contains the name of the granular properties this shorthand compacts.
  //
  ...
  //   Breaks the shorthand up to its granular properties.
  //
  var processable = {
    'margin': {
      components: [
  ...
  var compactToShorthands = function(tokens, important) {
    ...
    for (var i in processable) {
      ...
      for (var j = 0; j < sh.components.length; j++) {
        ...
        else if (tokens.some(importantTrickFunc)) {
      ...
      if (missingComps.length === 0 || detokenize([compacted]).length <= detokenize(comps).length || comps.some(isHackValue)) {
