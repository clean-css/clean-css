  ...
  var breakUp = {
    ...
    outline: function (token) {
      ...
      if (parts.length >= 1) {
        ...
        var w = parts.filter(function(p) { return p !== "inherit" && validator.isValidOutlineWidth(p); });
        if (w.length) {
          width.value = w[0];
      ...
      if (parts.length >= 1) {
        ...
        var w = parts.filter(function(p) { return p !== "inherit" && validator.isValidOutlineStyle(p); });
        if (w.length) {
          style.value = w[0];
          parts.splice(parts.indexOf(w[0]), 1);
        }
      ...
      if (parts.length >= 1) {
        ...
        var w = parts.filter(function(p) { return validator.isValidOutlineColor(p); });
        if (w.length) {
          color.value = w[0];
          parts.splice(parts.indexOf(w[0]), 1);
        }
