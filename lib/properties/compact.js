...
module.exports = (function () {
  ...
  var canMerge = {
    ...
    unit: function (t1, t2) {
      ...
      if (validator.isValidFunction(t2.value) && validator.isValidFunction(t1.value)) {
        var f1name = t1.value.substring(0, t1.value.indexOf('('));
        var f2name = t2.value.substring(0, t2.value.indexOf('('));
  ...
  var putTogether = {
    ...
    fourUnits: function (prop, tokens, isImportant) {
      ...
      if (tokens[0].isIrrelevant && tokens[2].isIrrelevant && tokens[1].value === tokens[3].value) {
        tokens[0].value = tokens[2].value = tokens[1].value;
      }
      if (tokens[1].isIrrelevant && tokens[3].isIrrelevant && tokens[0].value === tokens[2].value) {
        ...
        for (i = 0; i < tokens.length; i++) {
          if (tokens[i].value === 'inherit') {
            ...
            result2Shorthandable.push({
              prop: tokens[i].prop,
//              // Use shortest possible value, fallback to default value
//              value: processable[tokens[i].prop].shortestValue || processable[tokens[i].prop].defaultValue,
        ...
        // When some (but not all) of the tokens are 'inherit'
          ...
          var dr1 = detokenize(result1);
          var dr2 = detokenize(result2);
  ...
  var compactToShorthands = function(tokens, important) {
    ...
    for (var i in processable) {
      ...
      while (true) {
        ...
        var trickComps = [];
        // Array for tracking the non-tricked, authentic components
        var nonTrickComps = [];
        ...
        for (var j = 0; j < sh.components.length; j++) {
          ...
          if (found.length > 0) {
            ...
            comps.push(found[0]);
            nonTrickComps.push(found[0]);
            index = tokens.indexOf(found[0]);
            if (shorthandPosition === -1 || index < shorthandPosition) {
              shorthandPosition = index;
            }
          }
            ...
            var importantTrickComp = makeDefaultProperty(cc, important);
            importantTrickComp.isIrrelevant = true;
            comps.push(importantTrickComp);
            trickComps.push(importantTrickComp);
          }
          else if (lastCompacted && lastCompacted[0] && (lastCompacted[0].isIrrelevant || (lastCompacted[0].granularValues && lastCompacted[0].granularValues[cc]))) {
            // Another trick: when a previous shorthand sets a certain granular property, we can
            // hack together another shorthand if doesn't contain said granular property.
            var alreadyDefinedComp = makeDefaultProperty(cc, important, lastCompacted[0].granularValues[cc]);
            comps.push(alreadyDefinedComp);
            trickComps.push(alreadyDefinedComp);
          }
          else {
        ...
        // Put together the fine-grained properties
        // NOTE: the result can contain multiple new tokens
        var compacted = processable[i].putTogether(i, comps, important);
        if (!compacted) {
          // Failed to put together a shorthand, continue with the other properties
          continue shorthandsLoop;
        }
        ...
        var compactedLength = detokenize(compacted).length;
        var authenticLength = detokenize(nonTrickComps).length;
        ...
        if (trickComps.length === 0 || compactedLength < authenticLength || comps.some(isHackValue)) {
          // Put the compacted tokens in the correct place
          Array.prototype.splice.apply(tokens, [shorthandPosition, 0].concat(compacted));
        ...
        if (compactedLength >= authenticLength) {
          // In special cases some of the "tricky" algorithms above may be too aggressive,
          // this is here to prevent them from getting into an infinite loop.
          continue shorthandsLoop;
        }
  ...
  var process = function (input) {
    ...
    //console.log('\n> initial tokenization done');
    //console.log('\n> initial tokenization: ----------\n', JSON.stringify(tokens, 2, 2));
    //console.log('\n> initial tokenization: ----------\n', detokenize(tokens));
    
    tokens = breakUpShorthands(tokens);
    //console.log('\n> breaking up shorthands done');
    //console.log('\n> after breaking up shorthands: ----------\n', JSON.stringify(tokens, 2, 2));
    //console.log('\n> after breaking up shorthands: ----------\n', detokenize(tokens));
    
    tokens = mergeSame(tokens);
    //console.log('\n> merging same properties done');
    //console.log('\n> after merging same properties: ----------\n', JSON.stringify(tokens, 2, 2));
    //console.log('\n> after merging same properties: ----------\n', detokenize(tokens));
    
    tokens = compactToShorthands(tokens, false);
    //console.log('\n> compacting non-important shorthands done');
    //console.log('\n> after compacting non-important shorthands ----------\n', JSON.stringify(tokens, 2, 2));
    //console.log('\n> after compacting non-important shorthands ----------\n', detokenize(tokens));
    
    tokens = compactToShorthands(tokens, true);
    //console.log('\n> compacting important shorthands done');
