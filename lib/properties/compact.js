...
module.exports = (function () {
  ...
  // Regex for CSS units, disallows functions
  var cssUnitRegex = (/(\.?\d+(px|%|em|rem|in|cm|mm|ex|pt|pc|)|auto|inherit)/gi);
  var cssUnitRegexStr = cssUnitRegex.toString();
  var cssUnitRegexFull = new RegExp('^' + cssUnitRegexStr.substring(1, cssUnitRegexStr.length - 3) + '$', 'i');
  // Regex for CSS units, also allows functions (eg. calc) but disallows vendor prefixed functions
  var cssUnitRegexWithFunctions = (/(([A-Z]+(\-|[A-Z]|[0-9])+ *\(([A-Z]|[0-9]| |\+|\-|%|\.)*\))|(\.?\d+(px|%|em|rem|in|cm|mm|ex|pt|pc|))|auto|inherit)/gi);
  var cssUnitRegexWithFunctionsStr = cssUnitRegexWithFunctions.toString();
  var cssUnitRegexWithFunctionsFull = new RegExp('^' + cssUnitRegexWithFunctionsStr.substring(1, cssUnitRegexWithFunctionsStr.length - 3) + '$', 'i');
  // Regex for CSS units, also allows functions (eg. calc) and allows vendor prefixed functions
  var cssUnitRegexWithFunctionsPlusVendorPrefix = (/(((\-|[A-Z]|[0-9])+ *\(([A-Z]|[0-9]| |\+|\-|%|\.)*\))|(\.?\d+(px|%|em|rem|in|cm|mm|ex|pt|pc|))|auto|inherit)/gi);
  var cssUnitRegexWithFunctionsPlusVendorPrefixStr = cssUnitRegexWithFunctionsPlusVendorPrefix.toString();
  var cssUnitRegexWithFunctionsPlusVendorPrefixFull = new RegExp('^' + cssUnitRegexWithFunctionsPlusVendorPrefixStr.substring(1, cssUnitRegexWithFunctionsPlusVendorPrefixStr.length - 3) + '$', 'i');
  ...
  var validator = {
    ...
    isValidUnit: function (s) {
      return cssUnitRegexWithFunctions.test(s);
    },
  ...
  var canMerge = {
    ...
    unit: function (t1, t2) {
      ...
      // Understandability: (CSS unit without functions) > (CSS unit with functions) > anything else
      // NOTE: vendor-specific property values are not taken into consideration here at the moment
      
      if (cssUnitRegexFull.test(t2.value)) {
        console.log("1.", t2.value, "can override", t1.value);
        return true;
      }
      if (cssUnitRegexFull.test(t1.value)) {
        console.log("2.", t2.value, "can't override", t1.value);
        return false;
      }
      
      if (cssUnitRegexWithFunctionsFull.test(t2.value)) {
        console.log("3.", t2.value, "can override", t1.value);
        return true;
      } else { console.log("3.", "waaat?", cssUnitRegexWithFunctionsFull.test(t2.value)); }
      if (cssUnitRegexWithFunctionsFull.test(t1.value)) {
        console.log("4.", t2.value, "can't override", t1.value);
        return false;
      }
  ...
  var breakUp = {
    ...
    fourUnits: function (token) {
      ...
      var result = [];
      var splitval = token.value.match(cssUnitRegexWithFunctionsPlusVendorPrefix);
  ...
  var putTogether = {
    ...
    bySpacesOmitDefaults: function (prop, tokens, isImportant) {
      ...
      return {
        prop: prop,
        value: value.trim() || processable[prop].defaultValue,
        isImportant: isImportant
      };
    }
  };
