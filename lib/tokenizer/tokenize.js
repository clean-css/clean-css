...
function intoTokens(source, externalContext, internalContext, isNested) {
  ...
  for (; position.index < source.length; position.index++) {
    ...
    if (isEscaped) {
      ...
      buffer = [];
    } else if (character == Marker.OPEN_BRACE && level == Level.BLOCK && ruleToken && ruleToken[0] == Token.NESTED_BLOCK) {
      ...
      ruleToken = null;
    } else if (character == Marker.OPEN_BRACE && level == Level.BLOCK && tokenTypeFrom(buffer) == Token.NESTED_BLOCK) {
      // open brace opening at-rule at block level, e.g. @media{<--
      serializedBuffer = buffer.join('').trim();
